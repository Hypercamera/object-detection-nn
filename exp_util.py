...
#     return keras.optimizers.RMSprop(lr=lr_mult)# / (5 * nb_layers))
    return optimizers.Adam(lr=lr_mult)# / (5 * nb_layers))
...
def generate_conv_net_base_old(
    ...
    # FIXME workaround: using `padding` instead of `border_mode` to avoid errors
    # subsample --> strides
    model.add(Convolution2D(filters=nb_filters, kernel_size=kernel_size,
                            padding='same',
#                             border_mode='same',
...
#     model.add(Convolution2D(nb_filters, 3, 3, border_mode='same', activation='relu'))
    ...
    for i in range(nb_layers - 1):
        ...
        # FIXME workaround: using `padding` to replace `border_mode` to avoid errors
...
def generate_conv_net_base(
    ...
    # FIXME workaround: using `padding` to replace `border_mode` to avoid errors
    model.add(Convolution2D(filters=nb_filters, kernel_size=(kernel_size[0], kernel_size[1]),
                            padding='same',
                            input_shape=input_shape,
                            strides=stride))
    ...
    # FIXME workaround: using `padding` to replace `border_mode` to avoid errors
        model.add(Convolution2D(filters= nb_filters * nb_filter_multiplier, kernel_size=(3, 3), padding='same'))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(BatchNormalization())
        model.add(Activation("relu"))
        model.add(Dropout(0.25))
...
def get_csv_samples_many(csv_fnames, img_nums, OBJ, limit, begin=0, WINDOW=3, thre=40, crop=None):
    ...
    # FIXME original
    #counts = np.array(counts)
    # quick fix by wrapping it by list
...
#     print score_fname
    ...
    if cfg['generic']:
        ...
        model_fname += '.hdf5'
        model = tf.keras.models.load_model(str(model_fname))
    for i in range(video_clip_num):
        if not cfg['generic']:
